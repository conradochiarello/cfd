...
% SOR Algorithm for Diffusion 1D Steady-State without
...
while (1)
    for i = 1 : N
        if i == 1
            ...
            T_old(i) = T(i);
            T(i) = w*(a_e(i)*T(i+1) + S_u(i))/a_p(i) + (1-w)*T_old(i);
        ...
        elseif i < N
            ...
            T_old(i) = T(i);
            T(i) = w*(a_e(i).*T(i+1) + a_w(i).*T(i-1) + S_u(i))./a_p(i) + (1-w)*T_old(i);
        ...
        elseif i == N
            ...
            T_old(i) = T(i);
            T(i) = w*(a_w(i)*T(i-1) + S_u(i))/a_p(i) + (1-w)*T_old(i);
        end
    end
